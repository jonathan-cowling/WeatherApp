...
class LoadingFragmentTest {
  ...
  fun setup() {
    ...
    (InstrumentationRegistry.getTargetContext().applicationContext as WeatherApplication)
      .component = DaggerLoadingFragmentTest_TestComponent.builder().testModule(testModule)
  ...
  fun testFragmentHasCorrectText() {
    ...
    LoadingFragment().show(activityTestRule.activity.supportFragmentManager, "")
    onView(withId(R.id.loading_message_detailed))
      .check(matches(allOf(withText("Getting weather in Manchester"))))
